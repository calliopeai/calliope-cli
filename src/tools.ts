...
import { scopeManager, validatePath as scopeValidatePath, isInScope, getScopeSummary } from './scope.js';
...
- defaultProvider: AI provider (anthropic, google, openai, together, openrouter, groq, fireworks, mistral, ollama, huggingface, deepseek, xai, cerebras, litellm, bedrock, auto)
...
function validatePath(filePath: string, cwd: string): string {
  ...
  const canonical = scopeManager.canonicalize(absResolved);
  const canonicalState = scopeManager.canonicalize(cliStateDir) ?? cliStateDir;
  if (absResolved === cliStateDir || absResolved.startsWith(cliStateDir + path.sep) ||
    canonical && (canonical === canonicalState || canonical.startsWith(canonicalState + path.sep))) {
      ...
      case 'configure': {
        ...
        if (action === 'list') {
          ...
          if (category === 'providers' || category === 'all') {
            const providers = ['anthropic', 'google', 'openai', 'together', 'openrouter', 'groq', 'fireworks', 'mistral', 'ollama', 'huggingface', 'deepseek', 'xai', 'cerebras', 'litellm', 'bedrock', 'auto'];
