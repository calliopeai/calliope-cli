...
export function readRunAccounting(store:ExecutionStore,execution:ExecutionInspection=store.read()):RunAccounting {
  try {
    const {budget}=inspectSpawnAuthority(store,new ReservationLedger(join(store.root,'..','budget')),execution);
    ...
    return{version:1,status:'available',basis:'reservations-and-settlements',revision:state.revision,executionRevision:execution.state.revision,exceeded:state.exceeded,run,accounts};
