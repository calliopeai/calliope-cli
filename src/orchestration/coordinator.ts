...
export async function executeReviewedRun(cwd:string,runId:string,options:CoordinatorOptions={}):Promise<CoordinatorResult> {
  ...
  const work=async(task:ProjectTask):Promise<void>=>{
    ...
    try {
      ...
      const agentOutputCap=agent.id===context.plan.supervision?.controllerId?Math.min(512,agent.tokenBudget):agent.costBudgetUsd<=0.1?Math.min(4096,agent.tokenBudget):agent.tokenBudget;
