...
export async function importKnowledgeGraph(cwd: string, path: string, options: BrainOptions = {}) {
  ...
  for (const node of graph.nodes) {
    const props = sanitizeBrainValue(node.props), rawAttributes = typeof props.attributes === 'object' && props.attributes && !Array.isArray(props.attributes) ? props.attributes as Record<string, unknown> : {}, attributes: Record<string, string | number | boolean | null> = { externalId: node.id, externalType: node.type };
    for (const [key, value] of Object.entries(rawAttributes)) if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean' || value === null) attributes[key] = value;
    changes.push({ kind: 'entity', id: namespace(node.id), expected: prior.state.revisions['entity:' + namespace(node.id)] ?? null, value: { id: namespace(node.id), kind: (['document','decision','requirement','task','risk','dependency','stakeholder','provider','agent','artifact','test_evidence','run'] as const).includes(node.type as never) ? node.type as never : 'artifact', name: sanitizeBrainText(node.name), summary: typeof props.summary === 'string' ? sanitizeBrainText(props.summary) : sanitizeBrainText(node.name), state: 'proposed', confidence: typeof props.confidence === 'number' ? Math.max(0, Math.min(1, props.confidence)) : 0.5, provenance: [{ sourceId, basis: 'observed' }], attributes } });
  }
