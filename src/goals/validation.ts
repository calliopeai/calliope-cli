...
export function validateGoalManifest(value:unknown):GoalManifest {
  ...
  const allowedTools=value.version===3?[...GOAL_TOOLS,'shell']:GOAL_TOOLS;
  shape(value.workspace,['allowedTools','allowedPaths']);strings(value.workspace.allowedTools,allowedTools.length);if(value.workspace.allowedTools.some(t=>!allowedTools.includes(t)))fail('Goal tools require supported containment.');if(value.version===3&&!value.workspace.allowedTools.includes('shell'))fail('Supervised goals require contained verification authority.');array(value.workspace.allowedPaths,256);
