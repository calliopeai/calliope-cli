...
export function validateGoalManifest(value:unknown):GoalManifest {
  ...
  shape(value.workspace,['allowedTools','allowedPaths']);strings(value.workspace.allowedTools,GOAL_TOOLS.length);if(value.workspace.allowedTools.some(t=>!GOAL_TOOLS.includes(t)))fail('Goal tools require supported containment.');array(value.workspace.allowedPaths,256);
