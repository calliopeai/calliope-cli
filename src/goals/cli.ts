...
export async function runGoalCommand(args:string[],options:GoalCommandOptions={}):Promise<number> {
  ...
  try {
    let parsed:ReturnType<typeof parseArgs>;try{parsed=parseArgs({args,allowPositionals:true,tokens:true,options:{...stringFlags,brain:{type:'boolean'},supervise:{type:'boolean'},json:{type:'boolean'},'allow-mutations':{type:'boolean'},'read-path':{type:'string',multiple:true},'write-path':{type:'string',multiple:true}}});}catch{throw new OrchestrationError('invalid',GOAL_USAGE);}
    ...
    if(v.routing!==undefined&&v.routing!=='smart'||v['routing-policy']!==undefined&&v.routing!=='smart'||v.routing==='smart'&&v['routing-policy']===undefined&&!options.routing)throw new OrchestrationError('invalid','Use --routing smart --routing-policy <project-relative JSON file>.');
    const routing=v['routing-policy']!==undefined?await loadGoalRouting(cwd,String(v['routing-policy']),options):options.routing;
    const opts:GoalOptions={...options,goals,limits,...(v.brain?{brain:true}:{}),...(routing?{routing}:{}),...(v['planning-repairs']!==undefined?{planningRepairs:Number(v['planning-repairs'])}:{}),...(supervision?{supervision}:{}),...(configuredTeam?{team:configuredTeam}:{}),...(v.provider||v.model?{preference:{provider:String(v.provider??options.preference?.provider??'auto'),...(v.model?{model:String(v.model)}:{})}}:{}),...(v['read-path']||v['write-path']?{workspace:{allowedTools:['think','read_file','list_files',...(v['write-path']?['write_file','edit_file']:[])],allowedPaths:[...(v['read-path'] as string[]??[]).map(path=>({path,access:'read' as const})),...(v['write-path'] as string[]??[]).map(path=>({path,access:'write' as const}))]}}:{}),...(execution&&limits.maxOutputTokens!==undefined?{maxOutputTokens:limits.maxOutputTokens}:{}),...(v['allow-mutations']?{approve:async()=> 'allow' as const}:{}),
