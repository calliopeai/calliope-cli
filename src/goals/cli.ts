...
const stringFlags=Object.fromEntries([...Object.keys(numeric),...supervisionFlags,'provider','model','planner-provider','planner-model','worker-provider','worker-model','reviewer-provider','reviewer-model','attempts'].map(key=>[key,{type:'string' as const}]));
