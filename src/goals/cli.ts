...
const stringFlags=Object.fromEntries([...Object.keys(numeric),...supervisionFlags,'provider','model','planner-provider','planner-model','worker-provider','worker-model','reviewer-provider','reviewer-model','attempts','planning-repairs'].map(key=>[key,{type:'string' as const}]));
...
export function formatGoal(value:GoalResult,action='status'):string {
  ...
  if(m.team||m.supervision)lines.push(`${m.supervision?'Planner':'Planner/controller'}: ${m.preference.provider}:${m.preference.model??'auto'} · workers: ${m.team?.workers?m.team?.workers.provider+':'+(m.team?.workers.model??'auto'):'inherit'} · reviewer: ${m.team?.reviewer?m.team?.reviewer.provider+':'+(m.team?.reviewer.model??'auto'):'none'} · maximum task attempts: ${m.team?.maxAttempts??'per plan'}`);
  if(m.supervision){const s=m.supervision,controller=s.controller??m.preference;lines.push(`Supervision: ${s.principle} · controller ${controller.provider}:${controller.model??'auto'} · execution reviewer ${s.reviewer?s.reviewer.provider+':'+(s.reviewer.model??'auto'):'none'} · rounds ${s.maxRounds} · stalled rounds ${s.maxStalledRounds} · output limit ${s.maxOutputTokens}`,`Verification image: ${s.image} · local Docker · network disabled · read-only mounts`);}
...
export async function runGoalCommand(args:string[],options:GoalCommandOptions={}):Promise<number> {
  ...
  try {
    let parsed:ReturnType<typeof parseArgs>;try{parsed=parseArgs({args,allowPositionals:true,tokens:true,options:{...stringFlags,supervise:{type:'boolean'},json:{type:'boolean'},'allow-mutations':{type:'boolean'},'read-path':{type:'string',multiple:true},'write-path':{type:'string',multiple:true}}});}catch{throw new OrchestrationError('invalid',GOAL_USAGE);}
    ...
    if(v['planning-repairs']!==undefined&&!/^[0-2]$/.test(String(v['planning-repairs'])))throw new OrchestrationError('invalid','--planning-repairs must be between 0 and 2.');
    const configuredTeam=Object.keys(team).length>1?validateGoalTeam(team):undefined;
    const supervision=goalSupervisionFlags(v,options.supervision);
