...
async function planGoal(cwd:string,id:string,options:GoalOptions):Promise<GoalResult> {
  await owned(cwd,id,options,async context=>{
    ...
    try{
      const run=await ensureRun(cwd,view,view.state.planning!,plan,context),runs=runStore(view,options),executionStore=new ExecutionStore(join(runs.root,run.run.id),run.manifest),execution=await executeReviewedRun(cwd,run.run.id,{...context.options,measuredInputReservation:true,store:runs,confirmation:'none',resume:executionStore.exists(),maxOutputTokens:Math.min(view.manifest.limits.maxOutputTokens,options.maxOutputTokens??view.manifest.limits.maxOutputTokens),onEvent:options.onRunEvent});
