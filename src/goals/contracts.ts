...
export interface GoalConfiguration {limits?:Partial<GoalLimits>;workspace?:GoalManifest['workspace'];preference?:GoalManifest['preference'];team?:GoalTeam;supervision?:GoalSupervision;planningRepairs?:number;routing?:GoalRouting}
...
export function plannerPlan(manifest:GoalManifest):ProjectPlan {
  const checked=validateGoalManifest(manifest),m:GoalManifest=checked.version>=4?JSON.parse(canonicalJson(checked)):checked,l=m.limits,allowedTools=m.workspace.allowedTools.filter(t=>['think','read_file','list_files'].includes(t)),allowedPaths=m.workspace.allowedPaths.map(g=>({path:g.path,access:'read' as const}));
