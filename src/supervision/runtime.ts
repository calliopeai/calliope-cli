...
async function controllerTurn(store:ExecutionStore,authority:AgentExecution,role:SupervisionRole,options:CoordinatorOptions,assertRun:()=>void):Promise<void> {
  ...
  try {
    ...
    if(result.reason!=='completed'){
      ...
      const mechanicallyComplete=role==='controller'&&result.reason==='length'&&outcomes.length>0&&outcomes.every(outcome=>Array.isArray(outcome.checks)&&outcome.checks.length>0&&outcome.checks.every(check=>check.passed));
      if(!mechanicallyComplete)throw new OrchestrationError(result.reason==='budget'?'policy-denied':'unavailable',`${role==='reviewer'?'Reviewer':'Controller'} stopped: ${result.reason}.`);
