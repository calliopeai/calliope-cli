...
export function agentFiles(guard:ExecutionGuard,agentId:string,call:ToolCall,signal?:AbortSignal):FsDelegate {
  ...
  return {
    ...
    writeTextFile:async(file,content)=>{
      ...
      const parent=dirname(file),missing:string[]=[];let dir=parent;
      ...
      for(const path of missing.reverse()){
        if(!permits(account.allowedPaths,relative(guard.manifest.project.root,path),'write')||canonicalPath(path)!==path)throw new ExecutionLimitError('authority','Creating a parent directory exceeds the agent write grant.');
      ...
      try{
        ...
        fs.renameSync(temp,file);const directory=fs.openSync(parent,'r');try{fs.fsyncSync(directory);}finally{fs.closeSync(directory);}
    ...
    listFiles:async(file,recursive)=>{
      ...
      const walk=(dir:string,depth:number)=>{
        check(dir);if(depth>5||count>=1000)return;const handle=fs.opendirSync(dir);
