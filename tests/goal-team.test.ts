...
it('keeps leaf workers large enough for a bounded verification turn',()=>{
  ...
  expect(proposed.plan.agents.filter(agent=>agent.parentId!==null).map(agent=>agent.tokenBudget)).toEqual([25000,25000]);
  expect(proposed.plan.agents[0]!.tokenBudget).toBe(50000);
});
