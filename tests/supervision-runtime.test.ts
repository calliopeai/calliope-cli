...
it('reviews actual executor artifacts with distinct controller/reviewer models, updates the HUD and replays without calls',async()=>{
  ...
  expect(result.execution.state).toMatchObject({version:6,supervision:{rounds:1,phase:'ready'}});expect(hud.some(row=>row.includes('reviewing round 1'))).toBe(true);expect(hud.some(row=>row.includes('review health 1/1 sampled')),hud.join('\n')).toBe(true);expect(hud.some(row=>row.includes('review health 0% err'))).toBe(true);
