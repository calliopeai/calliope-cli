...
it('adopts approved children while independent workers are still running and verifies all added artifacts',async()=>{
  ...
  const [result,child]=await Promise.all([running,executeSpawn(project,accepted.admission,{store:runs})]);expect(result.status).toBe('completed');expect(child.status).toBe('completed');expect(result.execution.state.tasks['inspect-c']!.output!.testEvidence).toEqual(['output-check']);expect(requests).toHaveLength(8);expect(result.accounting).toMatchObject({status:'available',run:{accounted:{tokens:80}},accounts:{c:{accounted:{tokens:20}},coordinator:{accounted:{tokens:80}}}});
...
it('cancels a waiting spawned subtree, retains its charged request, and permits a bounded explicit retry',async()=>{
  ...
  expect(first.status).toBe('partial');expect(first.execution.state.tasks['inspect-c']!.status).toBe('cancelled');expect(first.accounting).toMatchObject({status:'available',accounts:{c:{requests:{unknown:1}}}});expect(first.execution.state.tasks['inspect-a']!.status).toBe('completed');
